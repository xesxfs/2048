	...
	export module game {
		export class Game extends egret.HashObject {
			...
			private addStartTiles() {
				for (var i: number = (0); i < this._startTiles; i++) {
					this.addRandomTile();
				}
			}
			...
			protected onMouseUp(event: egret.TouchEvent) {
				var kbEvent: KeyEvent
			...
			protected onMouseDown(event: egret.TouchEvent) {
			...
			private buildTraversals(vector: egret.Point): any {
				...
				for (var pos: number = (0); pos < this._size; pos++) {
			...
			private tileMatchesAvailable(): boolean {
				...
				for (var x: number = (0); x < this._size; x++) {
